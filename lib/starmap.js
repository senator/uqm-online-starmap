define(["jquery", "timer", "triggers_interface"],
  ...
  function StarMap() {
    ...
    this._init = function(underlay, canvas, overlay) {
      ...
      /* XXX not here for at least two reasons: 1) race condition with loading
       * data, 2) need to avoid stacking event listeners, 3) neatness. */
      overlay.addEventListener("click",
          function(evt) { self.log_near_star(evt.clientX, evt.clientY); });
    ...
    /* XXX temporary? */
    this.log_near_star = function(event_x, event_y) {
      var converted = this.map_coordinates(event_x, event_y);
      var star =
        this.find_nearest_star(converted[0], converted[1], 100 /* XXX should scale with canvases */);

      if (star) {
        var d = this.describe_star(star).display;
        console.log(d.name + ":", d.type, "at", d.x, "x", d.y);
      } else {
        console.log("Nothing close enough to your click");
      }
    };
