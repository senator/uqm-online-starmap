define(["jquery", "timer", "triggers_interface"],
  ...
  function StarMap() {
    ...
    this._init = function(root_el) {
      ...
      this.elements = this.find_required_elements(root_el);

      /* Kludgey, but this object manipuates the canvas element directly,
       * while letting other objects manage the other elements, so it saves
       * typing and a lookup. */
      this.canvas = this.elements.canvas;
      this.star_ctx = this.canvas.getContext("2d");

      /* XXX TODO attach these three calls to a window resize event */
      this.set_canvas_waste_and_position();
      this.size_canvases_to_viewport();
      this.set_map_scale();

      this.grid = new StarMapGrid(this.elements.underlay, this.scale);

      /* XXX obviously, define the method given as a callback to StarMapOverlay
       * somewhere else.
       *
       * ALSO: This is too early to activate it yet.  You can mouseover a star
       * before the stars are all loaded, and get JS errors.
       * */
      this.overlay = new StarMapOverlay(this.elements.overlay, function(evt) {
    ...
    this.find_required_elements = function(root) {
      var elements = {};
    ...
    this.size_canvases_to_viewport = function()  {
      var self = this;
      var vp = document.documentElement; /* Stand-in for viewport. */

      /* XXX This should give us a square of the correct size in either
       * landscape or portrait orientation, but landscape won't look
       * right at least until I put in the media query stylesheets.
       */
      var pixels = (vp.clientWidth > vp.clientHeight) ?
        Math.floor(vp.clientWidth * 0.76) - this.canvas_x_waste :
        Math.floor(vp.clientHeight * 0.76) - this.canvas_y_waste;

      CANVAS_NAMES.forEach(function(name) {
        self.elements[name].width = self.elements[name].height = pixels;
      });
     ...
     * all the time.  */
    this.set_canvas_waste_and_position = function() {
      var rect = this.canvas.getBoundingClientRect();

      this.canvas_x_waste = (rect.width - this.canvas.width) / 2;
      this.canvas_y_waste = (rect.height - this.canvas.height) / 2;
    ...
    this.map_coordinates = function(event_x, event_y) {
      var canvas_x = (event_x - this.canvas_x_waste) - this.canvas_left;
      var canvas_y = (event_y - this.canvas_y_waste) - this.canvas_top;
